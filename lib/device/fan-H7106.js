...
export default class {
  constructor(platform, accessory) {
    ...
    this.swingCodes = {
      0: 'qh8AAAAAAAAAAAAAAAAAAAAAALU=',
      1: 'qh8BAAAAAAAAAAAAAAAAAAAAALQ=',
    }
    ...
    //
    // 🔧 STEP 1: Send swing ON code 5 seconds after startup (manual test)
    //
    setTimeout(async () => {
      try {
        await this.platform.sendDeviceUpdate(this.accessory, {
          cmd: 'ptCmd', // Changed from 'ptReal' to 'ptCmd' here
          value: this.swingCodes[1], // swing ON
        })
        this.accessory.log('[Manual Test] Sent swing ON code directly')
      } catch (err) {
        this.accessory.logWarn(`[Manual Test] Failed to send swing ON: ${parseError(err)}`)
      }
    }, 5000)
  }
