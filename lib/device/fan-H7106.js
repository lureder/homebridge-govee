...
export default class {
  constructor(platform, accessory) {
    ...
    // Swing codes
    this.swingCodes = {
      0: 'Mx8BAAAAAAAAAAAAAAAAAAAAAC0=', // Swing Off
      1: 'Mx8BAQAAAAAAAAAAAAAAAAAAACw=', // Swing On
    }
  ...
  externalUpdate(params) {
    ...
    (params.commands || []).forEach((command) => {
      ...
      switch (deviceFunction) {
        ...
        case '1f01': { // Hypothetical swing feedback code
          const swingStatus = getTwoItemPosition(hexParts, 4)
          const newSwing = swingStatus === '01' ? 1 : 0
          if (newSwing !== this.cacheSwing) {
            this.cacheSwing = newSwing
            this.service.updateCharacteristic(this.hapChar.SwingMode, this.cacheSwing)
            this.accessory.log(`${platformLang.curSwing} [${this.cacheSwing === 1 ? 'on' : 'off'}]`)
          }
          break
        }
        default:
