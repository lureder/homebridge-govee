...
export default class {
  ...
  async internalSwingUpdate(value) {
    try {
      ...
      // carrying over the extra bytes from the last report seen (#1334).
      // Mirroring a status code is not guaranteed on every model, so the
      // documented OpenAPI oscillation toggle is also sent when the device
      // advertises it
      const tailBytes = this.cacheSwingCode
