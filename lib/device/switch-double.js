...
// Models whose night light can be controlled. The H5089 reports its light as
// `aa 1b 01 [on] [level]` and that reading is correct, but no command shape
// tried so far moves it - neither mirroring its own status code nor the
// `33 18` the purifiers use - so it is left out rather than shipping a tile
// that reports properly and ignores every tap (#1323). Put the model back
// here once a command is known to work.
const nightLightModels = []
