...
export default class {
  constructor(platform, accessory) {
    ...
    // Only offer the controls this model actually has, so a heater without a
    // turning head does not get an oscillation switch that does nothing
    const resolved = resolveControls({
      model: this.accessory.context.gvModel,
      family: 'heater1',
      firmware: this.accessory.context.firmware,
      openApiCapabilities: this.accessory.context.openApiCapabilities,
      deviceConf: platform.deviceConf?.[this.accessory.context.gvDeviceId],
    })
    this.features = resolved.controls
    ...
    } else if (this.service.testCharacteristic(this.hapChar.LockPhysicalControls)) {
      this.service.removeCharacteristic(this.service.getCharacteristic(this.hapChar.LockPhysicalControls))
    }
  ...
  externalUpdate(params) {
    ...
    if (hasProperty(params, 'temperature')) {
      ...
      if (newTemp <= 100) {
        // A real reading, so this heater does report the room temperature and
        // belongs on the other tile. Remember that for next time rather than
        // asking the owner to work it out and tick a box themselves. The tile
        // is not swapped mid-run, since that would rebuild the accessory
        // underneath HomeKit while it is in use.
        if (!this.accessory.context.supportsAmbientTemp) {
          this.accessory.context.supportsAmbientTemp = true
          this.accessory.logWarn('this heater reports the room temperature, so it will move to a temperature-capable tile the next time homebridge restarts')
        }
      }
    }
