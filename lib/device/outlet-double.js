...
export default class {
  ...
  async internalLightStateUpdate(value) {
    try {
      ...
      await this.platform.sendDeviceUpdate(this.accessory, {
        // The H5089 acks app commands as `ptIot`, and ignores the older
        // `ptReal` vehicle this code first shipped with (#1323)
        cmd: 'ptIot',
        value: generateCodeFromHexValues([0x33, 0x1B, 0x01, value ? 0x01 : 0x00, this.cacheLightBright]),
      })
  ...
  async internalLightBrightnessUpdate(value) {
    try {
      ...
      await this.platform.sendDeviceUpdate(this.accessory, {
        cmd: 'ptIot',
        value: generateCodeFromHexValues([0x33, 0x1B, 0x01, 0x01, value]),
      })
