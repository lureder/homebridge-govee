...
export default class {
  constructor(platform, accessory) {
    ...
    // Expose the main control as a simple Switch (proper On/Off behaviour),
// but categorize the accessory as a humidifier so HomeKit uses the humidifier icon/type.
const cats = this.platform?.api?.hap?.Categories
if (cats) {
  this.accessory.category = cats.HUMIDIFIER ?? cats.HUMIDIFIER_DEHUMIDIFIER ?? this.accessory.category
}

// Remove any prior HumidifierDehumidifier service that might exist from earlier versions of this file
const oldHum = this.accessory.getService(this.hapServ.HumidifierDehumidifier)
if (oldHum) {
  this.accessory.removeService(oldHum)
}

// Add the switch service if it doesn't already exist (this is the primary HomeKit tile)
...
this.cacheState = this.service.getCharacteristic(this.hapChar.On).value ? 'on' : 'off'  
    // --- Main: RotationSpeed (0..100%) ---
    // NOTE: RotationSpeed is not a standard characteristic for Switch.
  ...
  async internalStateUpdate(value) {
    try {
      ...
      if (this.cacheState === newValue) {
        // If we're turning on and speed is 0, still apply lastNonZeroLevel (HomeKit can send repeated On=true)
