...
export default class {
  constructor(platform, accessory) {
    ...
    // --- CLEAN: remove humidity-targeting characteristics so Home stops trying to show targets ---
    ...
    targetStateChar.onGet(() => this.hapChar.TargetHumidifierDehumidifierState.HUMIDIFIER)
    ...
    // --- Current state (keep INACTIVE to avoid “Rising to”) ---
    const currentStateChar = this.service.getCharacteristic(this.hapChar.CurrentHumidifierDehumidifierState)
    currentStateChar.onGet(() => this.hapChar.CurrentHumidifierDehumidifierState.INACTIVE)
    currentStateChar.updateValue(this.hapChar.CurrentHumidifierDehumidifierState.INACTIVE)
    ...
    // Initialize Active so Home doesn’t sit on “Updating…”
    ...
    // Provide a getter + initialize value to avoid “Updating…”
  ...
  async internalActiveUpdate(value) {
    try {
      ...
      // Keep current state INACTIVE (UI choice)
