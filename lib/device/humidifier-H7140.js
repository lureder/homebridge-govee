...
export default class {
  constructor(platform, accessory) {
    ...
    // Dehumidifier service (shows as a Dehumidifier in HomeKit)
    this.service =
    ...
    // Force Target state to "Dehumidifier"
    this.service
      .getCharacteristic(this.hapChar.TargetHumidifierDehumidifierState)
      .updateValue(this.hapChar.TargetHumidifierDehumidifierState.DEHUMIDIFIER)
    ...
    // Current state (best-effort): inactive when off, dehumidifying when on
    this.service
      .getCharacteristic(this.hapChar.CurrentHumidifierDehumidifierState)
      .updateValue(
        this.cacheActive === 'on'
          ? this.hapChar.CurrentHumidifierDehumidifierState.DEHUMIDIFYING
          : this.hapChar.CurrentHumidifierDehumidifierState.INACTIVE,
      )

    // SPEED slider (0–100%, step 1)
    // Some HomeKit UIs may not show this on a dehumidifier, but it's still a valid characteristic.
    const speedChar =
  ...
  async internalSpeedUpdate(value) {
    try {
      ...
      // Ensure device is ON if user sets a non-zero speed
