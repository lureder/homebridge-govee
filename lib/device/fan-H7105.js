...
const POWER_ON_ECHO_WINDOW_MS = 2000
    ...
    for (const command of params.commands || []) {
      ...
      switch (deviceFunction) {
        case '0501': {
          ...
          // If within the power-on echo window, this 0501 is the device reporting
          // its current speed as part of the power-on state dump â€” not a speed change.
          // Silently sync _lastBucket and leave the slider exactly where it was.
          if (Date.now() - this._powerOnAt < POWER_ON_ECHO_WINDOW_MS) {
            this._lastBucket = newSpeedBucket
            break
          }

          // Outside the window â€” genuine speed change from Govee app or physical
          // controls. Snap slider to the bucket-max percentage.
