...
export default class FanH7105 {
  ...
  setupFanService() {
    ...
    this.cacheSpeedBucket = this.speedPctToBucket(this.cacheSpeedPct)
  }
  ...
  async internalStateUpdate(value) {
    ...
    try {
      ...
      if (newState === 'off') {
        ...
        this.cacheSpeedBucket = 0
        this.charRotationSpeed.updateValue(0)
        this.accessory.log('Fan off')
      } else {
        this.accessory.log('Fan on')
      }
    } catch (err) {
  ...
  async internalSpeedUpdate(value) {
    ...
    try {
      ...
      if (this.cacheSpeedPct !== pct || this.cacheState !== 'on') {
        await this.sendDeviceUpdate({
          cmd: 'ptReal',
          value: this.speedCodes[bucket],
        })

        const prevBucket = this.cacheSpeedBucket
        const wasOff = this.cacheState !== 'on'

        this.cacheState = 'on'
        this.cacheSpeedPct = pct
        this.cacheSpeedBucket = bucket
      ...
      switch (deviceFunction) {
        case '0501': {
          ...
          if (this.cacheSpeedPct !== newSpeedPct) {
            ...
            this.cacheSpeedBucket = newSpeedBucket
            this.charRotationSpeed.updateValue(newSpeedPct)

            if (prevBucket !== newSpeedBucket) {
