  ...
  externalUpdate(params) {
    ...
    (params.commands || []).forEach((command) => {
      ...
      switch (deviceFunction) {
        case '0501': {
          // Fan speed (device report)
          const newSpeed = getTwoItemPosition(hexParts, 4)
          const newSpeedInt = Number.parseInt(newSpeed, 16)

          // If we just sent OFF, ignore ghost '1' reports for a short window
          if (this.lastUserOffAt && Date.now() - this.lastUserOffAt < 3000 && newSpeedInt <= 1) {
            break
          }
          ...
          // Otherwise, it's ON with a bucket 1â€“12
          if (this.cacheSpeed !== newSpeedInt || this.cacheState !== 'on') {
            this.cacheSpeed = newSpeedInt
            this.cacheState = 'on'
            this.service.updateCharacteristic(this.hapChar.Active, 1)
            // Show upper bound % on the slider
