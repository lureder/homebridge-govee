...
export default class FanH7105 {
  constructor(platform, accessory) {
    ...
    this._lastBucket = 0           // the last bucket we sent to the device
    this._ignoreNextSpeedReport = false // true after a power-on echo, so the
                                        // accompanying 0501 doesn't snap the slider
  ...
  async internalSpeedUpdate(value) {
    ...
    try {
      ...
      if (this._lastBucket !== bucket || this.cacheState !== 'on') {
        ...
        this.cacheState = 'on'
        this.cacheSpeedPct = pct // store the raw value the user set
  ...
  externalUpdate(params) {
    if (params.state && params.state !== this.cacheState) {
      ...
      // The device always follows a power-on state report with a 0501 speed report.
      // Flag it so we can silently sync _lastBucket without snapping the slider.
      if (this.cacheState === 'on') {
        this._ignoreNextSpeedReport = true
      }
    }
    ...
    for (const command of params.commands || []) {
      ...
      switch (deviceFunction) {
        case '0501': {
          ...
          if (this._ignoreNextSpeedReport) {
            // This is the speed report accompanying a power-on â€” just sync the
            // bucket silently and leave the slider exactly where it was.
            this._ignoreNextSpeedReport = false
            this._lastBucket = newSpeedBucket
            break
          }

          // A 0501 that arrives without a preceding power-on is a genuine speed
          // change from the Govee app or physical controls â€” snap to bucket max.
