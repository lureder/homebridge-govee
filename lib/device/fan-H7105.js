...
export default class FanH7105 {
  constructor(platform, accessory) {
    ...
    this._lastBucket = 0 // FIX: persists the last speed bucket across power cycles
  ...
  async internalStateUpdate(value) {
    ...
    try {
      if (newState === 'on') {
        // FIX: if we have a previous speed, send its code directly instead of a
        // generic POWER_ON_CODE. Speed codes power the device on AND set the speed,
        // so the device and HomeKit will always agree on the displayed value.
        if (this._lastBucket > 0) {
          await this.sendDeviceUpdate({
            cmd: 'ptReal',
            value: this.speedCodes[this._lastBucket],
          })
          const normalizedPct = this.bucketToSpeedPct(this._lastBucket)
          this.cacheState = 'on'
          this.cacheSpeedPct = normalizedPct
          this.charRotationSpeed.updateValue(normalizedPct)
          this.accessory.log(`${platformLang.curSpeed} [${this._lastBucket}]`)
        } else {
          // No prior speed known â€” fall back to generic power-on
          await this.sendDeviceUpdate({
            cmd: 'ptReal',
            value: POWER_ON_CODE,
          })
        ...
        // FIX: _lastBucket is intentionally NOT cleared here so it survives the
        // power-off and can be restored on the next power-on.
      }
    } catch (err) {
  ...
  async internalSpeedUpdate(value) {
    ...
    try {
      ...
      // FIX: normalize the cached percentage to the bucket-equivalent value so
      // cacheSpeedPct always matches what the device will report back via externalUpdate.
      const normalizedPct = this.bucketToSpeedPct(bucket)

      if (this.cacheSpeedPct !== normalizedPct || this.cacheState !== 'on') {
        await this.sendDeviceUpdate({
          cmd: 'ptReal',
          value: this.speedCodes[bucket],
        })

        this._lastBucket = bucket // FIX: remember the bucket across power cycles
        this.cacheState = 'on'
        this.cacheSpeedPct = normalizedPct // FIX: store normalized, not raw pct

        this.charActive.updateValue(1)
        this.charRotationSpeed.updateValue(normalizedPct) // FIX: snap slider to true value
