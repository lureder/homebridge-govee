...
export default class FanH7105 {
  ...
  async internalSpeedUpdate(value) {
    ...
    try {
      ...
      // Deduplicate on bucket — dragging within the same bucket range doesn't
      ...
      if (this._lastBucket !== bucket || this.cacheState !== 'on') {
        ...
        // Do not snap the slider — the user's value is what stays displayed.
    ...
    for (const command of params.commands || []) {
      ...
      switch (deviceFunction) {
        case '0501': {
          ...
          // Only update the slider if the device is reporting a bucket we didn't
          // set — i.e. an external change from the Govee app or physical controls.
          // If it matches _lastBucket, it's just the power-on state report for the
          // speed we already set, so we leave the slider at the user's value.
