...
export default class FanH7105 {
  ...
  try {
    ...
    const normalizedPct = this.bucketToSpeedPct(bucket) // ← ADD THIS

    if (this.cacheSpeedPct !== normalizedPct || this.cacheState !== 'on') { // ← compare against normalizedPct
      await this.sendDeviceUpdate({
        cmd: 'ptReal',
      ...
      this.cacheState = 'on'
      this.cacheSpeedPct = normalizedPct // ← store normalizedPct, not raw pct

      this.charActive.updateValue(1)
      this.charRotationSpeed.updateValue(normalizedPct) // ← snap the HomeKit slider
