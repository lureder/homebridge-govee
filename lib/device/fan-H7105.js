...
export default class FanH7105 {
  ...
  externalUpdate(params) {
    // Snapshot whether the fan was already on before processing this update.
    // Any 0501 command arriving in the same update as a power-on is just the
    // device reporting its current speed — not a speed change. We use this to
    // keep power and speed handling completely independent.
    const wasAlreadyOn = this.cacheState === 'on'
    ...
    for (const command of params.commands || []) {
      ...
      switch (deviceFunction) {
        case '0501': {
          ...
          if (!wasAlreadyOn) {
            // Fan just powered on — this 0501 is a state report, not a speed change.
            // Silently sync _lastBucket so future changes are detected correctly,
            // but leave the slider exactly where it was.
            this._lastBucket = newSpeedBucket
            break
          }

          // Fan was already on — this is a genuine speed change from the Govee
          // app or physical controls. Snap slider to the bucket-max percentage.
