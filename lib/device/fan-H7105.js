...
export default class FanH7105 {
  constructor(platform, accessory) {
    ...
    platform.log(
      ...
      JSON.stringify({ fanOnly: true, swingRemoved: true })
    )
  }
  ...
  removeLegacyServices() {
    ...
    const existingFanV2 = this.accessory.getService(this.hapServ.Fanv2)
    if (
      existingFanV2 &&
      existingFanV2.getCharacteristic(this.hapChar.RotationSpeed).props.unit === 'percentage'
    ) {
      this.accessory.removeService(existingFanV2)
    }
  ...
  speedPctToBucket(value) {
    ...
    return Math.max(1, Math.min(FAN_SPEED_LEVELS, Math.ceil(pct / (100 / FAN_SPEED_LEVELS))))
  }
