...
export default class FanH7105 {
  ...
  setupFanService() {
    ...
    // Initialise _lastBucket from the persisted characteristic value so that
    // after a Homebridge restart the first device state dump doesn't look like
    // an external speed change and snap the slider.
    this._lastBucket = this.speedPctToBucket(this.cacheSpeedPct)
  }
  ...
  async internalStateUpdate(value) {
    ...
    try {
      await this.sendDeviceUpdate({
        cmd: 'ptReal',
        value: value ? POWER_ON_CODE : POWER_OFF_CODE,
      })
  ...
  async internalSpeedUpdate(value) {
    ...
    try {
      ...
      if (this._lastBucket !== bucket || this.cacheState !== 'on') {
        ...
        this.cacheState = 'on'
        this.cacheSpeedPct = pct // preserve the exact user-set value

        this.charActive.updateValue(1)
        // slider is not touched â€” the user's value stays as-is
  ...
  externalUpdate(params) {
    // ---- Power state -------------------------------------------------------
    // Power on/off never affects the speed slider, regardless of source.
    ...
    for (const command of params.commands || []) {
      ...
      switch (deviceFunction) {
        case '0501': {
          ...
          // Only update the slider if the bucket differs from what we last set.
          // If it matches, this is a state dump echo (power-on report, periodic
          // poll, etc.) and the slider should be left exactly where it is.
          // If it differs, someone changed the speed from the Govee app or the
          // physical fan â€” snap the slider to the bucket-max percentage.
          if (newSpeedBucket !== this._lastBucket) {
            this._lastBucket = newSpeedBucket
            this.cacheSpeedPct = this.bucketToSpeedPct(newSpeedBucket)
            this.charRotationSpeed.updateValue(this.cacheSpeedPct)
