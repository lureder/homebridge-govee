...
describe('the status frames an H7123 sends on connect', () => {
  ...
  it('still warns about a frame it has never seen', () => {
    const device = build('H7123')
    // aa7f, which is not one of the known-and-ignored codes
    device.externalUpdate({ commands: ['qn8AAAAAAAAAAAAAAAAAAAAAANU='], source: 'AWS' })

    expect(device.accessory.logWarn.messages()).toHaveLength(1)
  })
  ...
  it('calls them status rather than scenes, because that is what they are', () => {
    ...
    const warned = device.accessory.logWarn.messages().join(' ')
    expect(warned).toContain('unrecognised status')
    expect(warned).not.toContain('scene')
  })
})
