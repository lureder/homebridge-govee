  ...
  externalUpdate(params) {
    ...
    if (hasProperty(params, 'temperature')) {
      ...
      if (newTemp !== this.cacheTemp) {
        if (newTemp > 100) {
          // Not a real reading, so this heater does not report the room
          // temperature and belongs on the simpler tile. Remember that for next
          // time rather than asking the owner to work it out themselves.
          if (this.accessory.context.supportsAmbientTemp !== false) {
            this.accessory.context.supportsAmbientTemp = false
            this.accessory.logWarn('this heater does not report the room temperature, so it will move to a simpler tile the next time homebridge restarts')
          }
        } else {
