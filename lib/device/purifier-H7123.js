...
export default class GoveePurifierH7123 extends GoveeDevice {
    ...
    try {
      ...
      await this.platform.sendDeviceUpdate(this.accessory, {
        ...
        openApi: this.accessory.context.openApiCapabilities?.workMode
          ? { instance: 'workMode', capabilityType: 'devices.capabilities.work_mode', value: { workMode: 1, modeValue: newValueKey } }
          : undefined,
      })
  ...
  externalUpdate(params) {
    ...
    if (params.workMode) {
      ...
      if (speed >= 1 && speed <= 5) {
        if (speed !== this.cacheMode) {
          this.cacheMode = speed
          this.service.updateCharacteristic(this.hapChar.RotationSpeed, this.cacheMode * 20)
          this.accessory.log(`${platformLang.curMode} [${this.value2Label[this.cacheMode]}]`)
        }
      }
    ...
    (params.commands || []).forEach((command) => {
      ...
      switch (deviceFunction) {
        ...
        case 'aa19': {
          ...
          // Govee reports 1-4 (excellent→poor); HomeKit AirQuality accepts 0-5.
          // Map Govee's 'poor' (and any higher/unexpected reading) to HomeKit's
          // Poor (5), and treat anything invalid as Unknown (0), so HomeKit is
          // never sent an out-of-range value (#1261).
          let newQual = Number.parseInt(getTwoItemPosition(hexParts, 5), 16)
          if (!Number.isInteger(newQual) || newQual < 1) {
            newQual = 0
          } else if (newQual >= 4) {
            newQual = 5
          }
