...
describe('connectByNameDarwin', () => {
  ...
  it('leaves no timer running once the device is found', async () => {
    // The scan deadline is up to 9s away but the device usually answers in well
    // under a second. Left pending, that timer holds the event loop open and
    // delays shutdown after every bluetooth command
    vi.useFakeTimers()
    try {
      const strip = fakePeripheral('Govee_H617A_325A')
      const promise = connection.connectByNameDarwin('db:48:c2:06:32:5a', 10000)
