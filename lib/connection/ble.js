...
export default class BLEConnection {
  ...
  handleDiscoveredPeripheral(peripheral) {
    try {
      ...
      // Skip if not a valid Govee sensor. Sensors whose broadcast the plugin
      // cannot decode yet are logged rather than dropped in silence, since the
      // hex is the one thing needed to add a model, and there is no other way
      // for an owner to get hold of it.
      if (!isValidPeripheral(peripheral)) {
        const unknownName = advertisement?.localName
        if (unknownName && unknownName.toLowerCase().startsWith('govee')) {
          const raw = advertisement.manufacturerData
            ? advertisement.manufacturerData.toString('hex')
            : 'no manufacturer data'
          const line = `[BLE] unrecognised govee sensor - please include this line if you report it: ${JSON.stringify({
            name: unknownName,
            address,
            source: 'BLE',
            kind: 'broadcast',
          ...
          // Say it once so it is actually noticed, then stay quiet about this
          // sensor for the rest of the session - it rebroadcasts constantly.
