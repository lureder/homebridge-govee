    ...
    try {
      ...
      if (httpDevices && httpDevices.length > 0) {
        ...
        httpDevices.forEach((httpDevice) => {
          ...
          // Skip Govee app device groups as these are not real devices (#1309)
          if (['BaseGroup', 'SameModeGroup'].includes(httpDevice.sku)) {
            this.log.debug('[%s] %s.', httpDevice.deviceName, platformLang.devIsGroup)
            return
          }
      ...
      if (openApiDevices && openApiDevices.length > 0) {
        openApiDevices.forEach((openApiDevice) => {
          // Skip Govee app device groups as these are not real devices (#1309)
          if (['BaseGroup', 'SameModeGroup'].includes(openApiDevice.model)) {
            this.log.debug('[%s] %s.', openApiDevice.deviceName, platformLang.devIsGroup)
            return
          }
      ...
      if (pendingLANDevices.length > 0) {
        ...
        pendingLANDevices.forEach((lanDevice) => {
          ...
          // Check it's not a matter-ignored device, if the config has been set, since a
          // device skipped by the http/openapi loops for this reason still has an
          // unmatched lan record, which would otherwise be added here and then removed
          // again by the cleanup below on every restart (#1315)
          if (this.config.ignoreMatter && platformConsts.matterModels.includes(lanDevice.sku)) {
            return
          }
      ...
      // Only treat 'not found in Govee' as a reason to remove an accessory when the
      // configured cloud clients actually returned their device lists this session,
      // otherwise a failed fetch would remove the accessory and destroy any HomeKit
      // scenes and automations it is part of (#1264)
      const cloudListsFetched = (!this.config.username || !!this.httpClient)
        && (!!this.config.openApiDisable || !this.config.apiKey || !!this.openApiClient)
      devicesInHB.forEach((accessory) => {
        // Remove the accessory if it is now being ignored, or if it is a matter model
        // and the user has enabled ignoreMatter (otherwise the cached accessory lingers
        // even though the device is skipped during setup) (#1287)
        const isIgnored = this.ignoredDevices.includes(accessory.context.gvDeviceId)
          || (this.config.ignoreMatter && platformConsts.matterModels.includes(accessory.context.gvModel))

        // Also remove it if the device no longer exists in Govee, but never based on
        // an incomplete cloud list, and never for LAN-only devices which may simply
        // have missed a scan window (#1264)
        const isMissing = !httpDevices.some(el => el.device === accessory.context.gvDeviceId)
        ...
        if (isIgnored || (isMissing && cloudListsFetched && !isLanOnly)) {
  ...
  initialiseDevice(device) {
    ...
    try {
      ...
      if (platformConsts.models.rgb.includes(device.model)) {
        ...
        // Device is a fan. Models whose speeds have not been worked out yet fall
        // back to the basic handler, which offers on and off and asks for what
        // it needs rather than leaving the device unsupported altogether
        devInstance = deviceTypes[`deviceFan${device.model}`] || deviceTypes.deviceFanBasic
        accessory = devicesInHB.get(uuid) || this.addAccessory(device)
      } else if (platformConsts.models.heater1.includes(device.model)) {
        // Some of these heaters report the room temperature and some do not,
        // and the two need different tiles in HomeKit - a plain fan against a
        // heater with a temperature readout. Rather than leave the owner to
        // work that out and tick a box, take Govee's own word for it where the
        // device says so, and otherwise use what a previous run learnt from the
        // readings themselves. The setting still wins if it has been set, so
        // nobody who has already configured this sees their tile change.
        const reportsTemperature = device.openApiInfo?.byInstance?.sensorTemperature
          ? true
          : devicesInHB.get(uuid)?.context?.supportsAmbientTemp
        devInstance = (deviceConf.tempReporting ?? reportsTemperature)
        ...
        doAWSPolling = true
        devInstance = deviceTypes[`deviceHumidifier${device.model}`] || deviceTypes.deviceHumidifierBasic
        accessory = devicesInHB.get(uuid) || this.addAccessory(device)
      } else if (platformConsts.models.dehumidifier.includes(device.model)) {
        // Device is a dehumidifier
        devInstance = deviceTypes[`deviceDehumidifier${device.model}`] || deviceTypes.deviceDehumidifierBasic
        doAWSPolling = true
        accessory = devicesInHB.get(uuid) || this.addAccessory(device)
      } else if (platformConsts.models.purifier.includes(device.model)) {
        // Device is a purifier
        devInstance = deviceTypes[`devicePurifier${device.model}`] || deviceTypes.devicePurifierBasic
        doAWSPolling = true
        accessory = devicesInHB.get(uuid) || this.addAccessory(device)
        ...
        devInstance = deviceTypes[`deviceIceMaker${device.model}`] || deviceTypes.deviceIceMakerBasic
        accessory = devicesInHB.get(uuid) || this.addAccessory(device)
      ...
      if (device.httpInfo) {
        ...
        if (device.httpInfo.deviceExt && device.httpInfo.deviceExt.deviceSettings) {
          ...
          try {
            ...
            this.log.debugWarn('[%s] failed to parse deviceSettings, skipping AWS/BLE setup.', accessory.displayName)
          }
