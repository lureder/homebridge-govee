...
export default class {
  ...
  pluginShutdown() {
    ...
    try {
      ...
      if (this.sensorStopDiscovery) {
        this.sensorStopDiscovery()
          .then(() => this.log('[BLE] stopped scanning for devices (bt-client).'))
      }
      if (this.bleClient) {
        this.bleClient.stopScan(() => this.log('[BLE] stopped scanning for devices (client).'))
      }
    } catch (err) {
