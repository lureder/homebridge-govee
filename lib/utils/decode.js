...
export function decodeH5179Values(streamUpdate) {
  ...
  // Temperature and humidity are each a little-endian pair, so the high byte is
  // read before the low one. They must not share a byte: the temperature pair
  // is bytes 6 and 7, the humidity pair bytes 8 and 9, and the battery byte 10.
  const temp_lsb = streamUpdate.substring(14, 16) + streamUpdate.substring(12, 14)
